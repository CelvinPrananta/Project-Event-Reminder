...
const ScheduleEventLine1 = () => {
  ...
  useEffect(() => {
    const eventsRef = ref(database, 'events1'); // Referensi untuk "events"
  ...
  const handleAddEvent = () => {
    ...
    const eventRef = ref(database, 'events1/' + (events.length + 1)); // Referensi baru untuk event
  ...
  const handleUpdateEvent = () => {
    ...
    const eventRef = ref(database, 'events1/' + editEvent.id); // Referensi untuk event yang akan diperbarui
  ...
  const handleDeleteEvent = () => {
    const eventRef = ref(database, 'events1/' + deleteEventId); // Referensi ke event yang akan dihapus
    remove(eventRef)
      .then(() => {
  ...
  return (
    <CCol xs={12}>
      <CCard className="mb-4">
        <CCardHeader>
          <strong>Schedule Event</strong> <small>Line 1</small>
        </CCardHeader>
        <CCardBody>
          <p className="text-body-secondary small">
            Please click <code>add event</code> to enter the event schedule for line 1.
          </p>
...
export default ScheduleEventLine1